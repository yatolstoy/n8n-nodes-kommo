...
import { IUpdateListElementForm, RequestListElementUpdate } from '../types';
...
export async function execute(
	...
	const listsCollection = (await this.getNodeParameter('collection', 0)) as IUpdateListElementForm;

	const body = listsCollection.element
		.map((element): RequestListElementUpdate => {
			return {
				...element,
